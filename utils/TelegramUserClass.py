...
class TelegramEntity():
    def __init__(self, username: str | None, full_name: str, telegram_id: int,
                 message_id: int) -> None:
        if username is None:
            self.username = "n/e"
        else:
    ...
    def __repr__(self):
        return f"""{self.username}, {self.full_name}, {self.telegram_id},
    {self.message_id}"""
...
class TelegramChat(TelegramEntity):
    def __init__(self, username: str | None, full_name: str, telegram_id: int,
                 message_id: int, chat_type: str) -> None:
        super().__init__(username, full_name, telegram_id, message_id)
        if chat_type:
...
class TelegramDeserialize():
    ...
    def unpack(message: types.Message, message_id_not_exists=False):
        ...
        if message_id_not_exists:
            user = TelegramUser(message.from_user.username,
                                message.from_user.full_name,
                                message.from_user.id,
                                message.chat.id, -1)
            chat = TelegramChat(message.chat.username,
