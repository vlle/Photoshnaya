...
async def del_admin(query: types.CallbackQuery, 
    ...
    await query.message.edit_text(text=msg["del_admin"]["delete_greet_adm"],
                                  parse_mode="HTML")
    data = {}
...
async def del_admin_accept_message(message: types.Message, bot: Bot,
    ...
    if (not message.forward_from or len(theme) > 1 or theme[0].lower() == 'отмена' 
        ...
        text = await i_del_admin(user_object=user,
                                 register=admin_unit,
                                 msg=msg)
