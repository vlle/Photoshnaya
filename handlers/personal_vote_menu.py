...
async def callback_next(query: CallbackQuery,
    ...
    if vote_db.is_user_not_allowed_to_vote(int(cb.group_id),
                                           int(cb.user)) is True:
        await bot.send_message(chat_id=int(cb.user),
                               text=msg["vote"]["already_voted"])
        return

    file_id = like_engine.select_next_contest_photo(int(cb.group_id),
...
async def callback_prev(query: CallbackQuery,
    ...
    file_id = like_engine.select_prev_contest_photo(int(cb.group_id),
                                                    int(cb.current_photo_id))
    photo_file_id = file_id[0]
    photo_id = file_id[1]
