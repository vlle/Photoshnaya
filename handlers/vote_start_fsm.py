...
async def set_vote(query: types.CallbackQuery, 
    ...
    photos = await admin_unit.select_contest_photos_ids(
            int(callback_data.group_id))
    amount_photo = len(photos)
    vote_announce = msg["vote"]["start_vote"].format(theme=theme,
                                                     amount_photo=amount_photo,
                                                     link=bot_link,
                                                     str_date="завтра вечером",
                                                     )
    await query.message.edit_text(text=msg["vote"]["will_you_post"] + vote_announce,
                                  parse_mode="HTML")
...
async def should_i_post_vote(message: types.Message,
                              bot: Bot,
                              state: FSMContext,
                              msg: dict):
