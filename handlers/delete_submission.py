...
async def delete_submission(query: types.CallbackQuery, 
    ...
    await query.message.edit_text(text=msg["delete_photo"]["create_greet_del"],
                                  parse_mode="HTML")
    data = {}
...
async def set_admin_delete_photo(message: types.Message, bot: Bot,
...
async def delete_photo_r_u_sure(bot: Bot,
                                state: FSMContext, admin_unit: AdminDB,
                                msg: dict):
    data: dict[str, Any] = await state.get_data()
    user = data["forward"]
    ...
    reply_board = types.ReplyKeyboardMarkup(keyboard=[[types.KeyboardButton(text="Да")],
    ...
    if photo_type == 'photo':
        ...
        await bot.send_document(data["user_id"],
                                caption=msg["delete_photo"]["are_you_sure"],
                                document=photo_file_id,
                                reply_markup=reply_board)
    await state.set_state(DeletePhoto.wait_for_confirmation)

async def make_delete_decision(message: types.Message, 
                                state: FSMContext, admin_unit: AdminDB,
                       msg: dict):
    if not message.text or message.text.lower() == 'нет':
        await message.reply(text=msg["delete_photo"]["cancel_del"],
        ...
        await message.reply(text=msg["delete_photo"]["del_photo"],
                            reply_markup=ReplyKeyboardRemove(remove_keyboard=True))
    else:
        await message.reply(text=msg["delete_photo"]["no_understand"],
                            reply_markup=ReplyKeyboardRemove(remove_keyboard=True))
        await state.clear()
