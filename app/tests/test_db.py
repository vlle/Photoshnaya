...
import pytest
from db.db_classes import Base, Photo, User
...
async def test_photo_change_persists(create_user, group, db):
    user = await create_user()
    register_unit = RegisterDB(db)
    m_group = ObjectFactory.build_group(group.group_name, group.group_id)

    await register_unit.register_photo_for_contest(
        user.telegram_id, m_group.telegram_id, file_get_id="file_1", type="photo"
    )

    async with AsyncSession(db) as session:
        stmt = select(Photo).join(User).where(User.telegram_id == user.telegram_id)
        photo = (await session.scalars(stmt)).one()
        assert photo.file_id == "file_1"
        assert photo.telegram_type == "photo"
        photo_id = photo.id

    status = await register_unit.register_photo_for_contest(
        user.telegram_id, m_group.telegram_id, file_get_id="file_2", type="video"
    )
    assert status == PhotoRegistrationStatus.CHANGED

    async with AsyncSession(db) as session:
        stmt = select(Photo).where(Photo.id == photo_id)
        photo = (await session.scalars(stmt)).one()
        assert photo.file_id == "file_2"
        assert photo.telegram_type == "video"
