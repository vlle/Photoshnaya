...
async def test_is_photo_registered_without_duplicating_submissions(create_user,
                                                                   group, db):
    users: list[User] = []
    for _ in range(0, 5):
        users.append(await create_user())
    ...
    for user in users:
        await register_unit.register_photo_for_contest(user.telegram_id,
                                                       m_group.telegram_id)
        await register_unit.register_photo_for_contest(user.telegram_id,
                                                       m_group.telegram_id)
    all_photo_ids = await register_unit.select_contest_photos_ids(
...
async def test_select_prev_contest_photo(create_user, group, db):
     ...
     _, next_id = await like.select_prev_contest_photo(m_group.telegram_id,
                                                       int(next_id))
